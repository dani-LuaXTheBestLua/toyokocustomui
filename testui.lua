...
local Library = {
    ...
    Scheme = {
        ...
        AccentColor = Color3.fromRGB(224, 224, 228),
    ...
    Registry = {},
    DPIRegistry = {},
    ConfigFolder = "ObsidianConfigs",
    ThemeFolder = "ObsidianThemes",
}

if RunService:IsStudio() then 
    if UserInputService.TouchEnabled and not UserInputService.MouseEnabled then
        Library.IsMobile = true
        Library.MinSize = Vector2.new(380, 260)
    else
        Library.IsMobile = false
        Library.MinSize = Vector2.new(520, 380)
    end
else
    pcall(function()
    ...
    Library.MinSize = Library.IsMobile and Vector2.new(380, 260) or Vector2.new(520, 380)
end
...
function Library:AddDraggableButton(Text: string, Func)
    ...
    Button.MouseEnter:Connect(function()
        TweenService:Create(Button, Library.TweenInfo, {
            BackgroundColor3 = Library.Scheme.Hover or Library.Scheme.MainColor
        }):Play()
    end)
    ...
    function Table:SetText(NewText: string)
        Label.Text = NewText
        local X = Library:GetTextBounds(NewText, Library.Scheme.Font, 14)
        Button.Size = UDim2.fromOffset(math.max(155, X + 40) * Library.DPIScale, 42 * Library.DPIScale)
        Library:UpdateDPI(Button, {
            Size = UDim2.fromOffset(math.max(155, X + 40), 42),
        })
    end
    Table:SetText(Text)
...
function Library:CreateWindow(WindowInfo)
    ...
    do
        ...
        --// Resize Handle (curved corner like screenshot)
        if WindowInfo.Resizable then
            ...
            -- Main curved grip (cream color matching screenshot)
            local Grip = New("Frame", {
                AnchorPoint = Vector2.new(1, 1),
                BackgroundColor3 = Color3.fromRGB(230, 210, 150),
            ...
            New("UICorner", {
                CornerRadius = UDim.new(1, 0), -- full round so it reads as a curve
                Parent = Grip,
            })

            -- Soft shadow under grip
            local GripShadow = New("Frame", {
                AnchorPoint = Vector2.new(1, 1),
    ...
    -- Better Open / Close animations
    local OpenInfo = TweenInfo.new(0.32, Enum.EasingStyle.Back, Enum.EasingDirection.Out)
    local CloseInfo = TweenInfo.new(0.2, Enum.EasingStyle.Quint, Enum.EasingDirection.In)
    ...
    function Library:Toggle(Value: boolean?)
        ...
        if typeof(Value) == "boolean" then
            if Library.Toggled == Value then return end
            ...
            pcall(function()
                ...
                for _, Option in pairs(Library.Options or {}) do
                    if Option.Type == "ColorPicker" then
                        if Option.ColorMenu then pcall(function() Option.ColorMenu:Close() end) end
                        if Option.ContextMenu then pcall(function() Option.ContextMenu:Close() end) end
                    elseif Option.Type == "Dropdown" or Option.Type == "KeyPicker" then
                        if Option.Menu then pcall(function() Option.Menu:Close() end) end
                    end
                end
            end)
        end
        ...
        if Library.Toggled then
            -- OPEN (scale up with slight overshoot)
            BaseSize = MainFrame.Size
            BasePos = MainFrame.Position

            MainFrame.Size = UDim2.fromOffset(BaseSize.X.Offset * 0.88, BaseSize.Y.Offset * 0.88)
            MainFrame.Position = UDim2.new(
                BasePos.X.Scale,
                BasePos.X.Offset + (BaseSize.X.Offset * 0.06),
                BasePos.Y.Scale,
                BasePos.Y.Offset + (BaseSize.Y.Offset * 0.06)
            )
            MainFrame.Visible = true

            local Tw = TweenService:Create(MainFrame, OpenInfo, {
                Size = BaseSize,
                Position = BasePos,
            })
            Tw:Play()
            ...
            -- CLOSE (scale down smooth)
            BaseSize = MainFrame.Size
            BasePos = MainFrame.Position

            local Tw = TweenService:Create(MainFrame, CloseInfo, {
                Size = UDim2.fromOffset(BaseSize.X.Offset * 0.9, BaseSize.Y.Offset * 0.9),
                Position = UDim2.new(
                    BasePos.X.Scale,
                    BasePos.X.Offset + (BaseSize.X.Offset * 0.05),
                    BasePos.Y.Scale,
                    BasePos.Y.Offset + (BaseSize.Y.Offset * 0.05)
                ),
            })
            Tw:Play()
            Tw.Completed:Connect(function()
    ...
    if Library.IsMobile then
        local ToggleButton = Library:AddDraggableButton("Toggle UI", function()
            Library:Toggle()
        end)
...
local function ManagerNotify(Title, Description)
    pcall(function()
        Library:Notify({Title = Title, Description = Description, Time = 3})
    end)
end

-- Create and Save are deliberately different:
-- Create = make a NEW JSON file and select it.
-- Save   = update the CURRENT/selected JSON file.
Library.CurrentConfig = Library.CurrentConfig or nil
Library.CurrentTheme = Library.CurrentTheme or nil
...
function Library:GetConfigs()
    ...
    local Success, Files = pcall(listfiles, Library.ConfigFolder)
...
function Library:GetConfigData()
    local Data = {Toggles = {}, Options = {}, Keybinds = {}}
    ...
    for Idx, Option in pairs(Options) do
        ...
        elseif Option.Type == "KeyPicker" then
            Data.Keybinds[Idx] = {Option.Value, Option.Mode}
        elseif Option.Type == "ColorPicker" then
            Data.Options[Idx] = {
                Option.Value:ToHex(),
                Option.Transparency or 0
            }
        end
    end
...
function Library:CreateConfig(Name)
    if not writefile then
        ManagerNotify("Config", "writefile is not supported")
        return false
    end

    Name = tostring(Name or ""):match("^%s*(.-)%s*$")
    if Name == "" then
        ManagerNotify("Config", "Enter a config name first")
        return false
    end
    ...
    if FileExists(Path) then
        ManagerNotify("Config", "\"" .. Name .. "\" already exists")
        return false
    end

    local Success, Encoded = JSONEncode(Library:GetConfigData())
    if not Success then
        ManagerNotify("Config", "Failed to encode config")
        return false
    end

    local Wrote = pcall(writefile, Path, Encoded)
    if not Wrote then
        ManagerNotify("Config", "Failed to create config")
        return false
    end

    Library.CurrentConfig = Name
    ManagerNotify("Config created", "\"" .. Name .. "\".json")
    return true
end
...
function Library:SaveConfig(Name)
    ...
    Name = tostring(Name or Library.CurrentConfig or ""):match("^%s*(.-)%s*$")
    if Name == "" then
        ManagerNotify("Config", "Create or load a config first")
        return false
    end
    ...
    if not FileExists(Path) then
        ManagerNotify("Config", "\"" .. Name .. "\" does not exist")
        return false
    end
    ...
    local Wrote = pcall(writefile, Path, Encoded)
    if not Wrote then
        ManagerNotify("Config", "Failed to save config")
        return false
    end

    Library.CurrentConfig = Name
    ManagerNotify("Config saved", "\"" .. Name .. "\"")
    return true
end

function Library:LoadConfig(Name)
    if not (readfile and isfile) then
        ManagerNotify("Config", "readfile/isfile is not supported")
        return false
    end

    Name = tostring(Name or ""):match("^%s*(.-)%s*$")
    if Name == "" then
        ManagerNotify("Config", "Select a config first")
        return false
    end

    local Path = Library.ConfigFolder .. "/" .. Name .. ".json"
    if not isfile(Path) then
        ManagerNotify("Config", "\"" .. Name .. "\" not found")
        return false
    end

    local ReadSuccess, Raw = pcall(readfile, Path)
    if not ReadSuccess then
        ManagerNotify("Config", "Failed to read config")
        return false
    end

    local DecodeSuccess, Data = JSONDecode(Raw)
    if not DecodeSuccess or typeof(Data) ~= "table" then
        ManagerNotify("Config", "Invalid JSON config")
        return false
    end
    ...
    if Data.Options then
        for Idx, Value in pairs(Data.Options) do
            if Options[Idx] then
                ...
                pcall(function()
                    if Opt.Type == "ColorPicker" and typeof(Value) == "table" then
                        Opt:SetValueRGB(Color3.fromHex(Value[1] or "FFFFFF"), Value[2] or 0)
                    else
                        Opt:SetValue(Value)
                    end
                end)
    ...
    if Data.Keybinds then
        for Idx, Value in pairs(Data.Keybinds) do
            if Options[Idx] and Options[Idx].Type == "KeyPicker" then
                pcall(function() Options[Idx]:SetValue(Value) end)
            end
        end
    end

    Library.CurrentConfig = Name
    ManagerNotify("Config loaded", "\"" .. Name .. "\"")
    return true
end

function Library:DeleteConfig(Name)
    if not delfile then
        ManagerNotify("Config", "delfile is not supported")
        return false
    end
    ...
    if Success then
        ...
        ManagerNotify("Config deleted", "\"" .. Name .. "\"")
        return true
    end

    ManagerNotify("Config", "Failed to delete \"" .. Name .. "\"")
    return false
end
...
function Library:GetThemes()
    local List = {}
    if not listfiles then return List end
    EnsureFolder(Library.ThemeFolder)

    local Success, Files = pcall(listfiles, Library.ThemeFolder)
    if Success and Files then
        for _, File in pairs(Files) do
            local Name = File:match("([^/\\]+)%.json$")
            if Name then table.insert(List, Name) end
...
local function ThemeRGB(Color)
    return {Color.R, Color.G, Color.B}
end

function Library:GetThemeData()
    return {
        BackgroundColor = ThemeRGB(Library.Scheme.BackgroundColor),
        MainColor = ThemeRGB(Library.Scheme.MainColor),
        AccentColor = ThemeRGB(Library.Scheme.AccentColor),
        OutlineColor = ThemeRGB(Library.Scheme.OutlineColor),
        FontColor = ThemeRGB(Library.Scheme.FontColor),
    }
end

function Library:CreateTheme(Name)
    if not writefile then
        ManagerNotify("Theme", "writefile is not supported")
        return false
    end

    Name = tostring(Name or ""):match("^%s*(.-)%s*$")
    if Name == "" then
        ManagerNotify("Theme", "Enter a theme name first")
        return false
    end

    EnsureFolder(Library.ThemeFolder)
    local Path = Library.ThemeFolder .. "/" .. Name .. ".json"

    if FileExists(Path) then
        ManagerNotify("Theme", "\"" .. Name .. "\" already exists")
        return false
    end

    local Success, Encoded = JSONEncode(Library:GetThemeData())
    if not Success then
        ManagerNotify("Theme", "Failed to encode theme")
        return false
    end

    local Wrote = pcall(writefile, Path, Encoded)
    if not Wrote then
        ManagerNotify("Theme", "Failed to create theme")
        return false
    end

    Library.CurrentTheme = Name
    ManagerNotify("Theme created", "\"" .. Name .. "\".json")
    return true
end
...
function Library:SaveTheme(Name)
    ...
    Name = tostring(Name or Library.CurrentTheme or ""):match("^%s*(.-)%s*$")
    if Name == "" then
        ManagerNotify("Theme", "Create or load a theme first")
        return false
    end
    ...
    if not FileExists(Path) then
        ManagerNotify("Theme", "\"" .. Name .. "\" does not exist")
        return false
    end
    ...
    local Wrote = pcall(writefile, Path, Encoded)
    if not Wrote then
        ManagerNotify("Theme", "Failed to save theme")
        return false
    end

    Library.CurrentTheme = Name
    ManagerNotify("Theme saved", "\"" .. Name .. "\"")
    return true
end

function Library:LoadTheme(Name)
    if not (readfile and isfile) then
        ManagerNotify("Theme", "readfile/isfile is not supported")
        return false
    end

    Name = tostring(Name or ""):match("^%s*(.-)%s*$")
    if Name == "" then
        ManagerNotify("Theme", "Select a theme first")
        return false
    end

    local Path = Library.ThemeFolder .. "/" .. Name .. ".json"
    if not isfile(Path) then
        ManagerNotify("Theme", "\"" .. Name .. "\" not found")
        return false
    end

    local ReadSuccess, Raw = pcall(readfile, Path)
    if not ReadSuccess then
        ManagerNotify("Theme", "Failed to read theme")
        return false
    end

    local DecodeSuccess, Data = JSONDecode(Raw)
    if not DecodeSuccess or typeof(Data) ~= "table" then
        ManagerNotify("Theme", "Invalid JSON theme")
        return false
    end
    ...
    if Data.BackgroundColor then Library.Scheme.BackgroundColor = ToColor(Data.BackgroundColor) end
    if Data.MainColor then Library.Scheme.MainColor = ToColor(Data.MainColor) end
    if Data.AccentColor then Library.Scheme.AccentColor = ToColor(Data.AccentColor) end
    if Data.OutlineColor then Library.Scheme.OutlineColor = ToColor(Data.OutlineColor) end
    if Data.FontColor then Library.Scheme.FontColor = ToColor(Data.FontColor) end

    pcall(function() Library:UpdateColorsUsingRegistry() end)

    Library.CurrentTheme = Name
    ManagerNotify("Theme loaded", "\"" .. Name .. "\"")
    return true
end

function Library:DeleteTheme(Name)
    if not delfile then
        ManagerNotify("Theme", "delfile is not supported")
        return false
    end
    ...
    local Success = pcall(delfile, Path)
    if Success then
        if Library.CurrentTheme == Name then Library.CurrentTheme = nil end
        ManagerNotify("Theme deleted", "\"" .. Name .. "\"")
        return true
    end

    ManagerNotify("Theme", "Failed to delete \"" .. Name .. "\"")
    return false
end
...
function Library:SetFont(FontFace)
    ...
    elseif typeof(FontFace) == "string" then
        local Ok, Result = pcall(function()
            return Font.fromEnum(Enum.Font[FontFace])
        end)
        if Ok then FontFace = Result end
    end
    ...
    if typeof(FontFace) == "Font" then
        ...
        for Instance, Properties in pairs(Library.Registry) do
            if Properties.FontFace or Properties.Font then
                pcall(function()
        ...
        ManagerNotify("Font", "Font updated")
    end
end

function Library:GetCurrentConfig()
    return Library.CurrentConfig
end
