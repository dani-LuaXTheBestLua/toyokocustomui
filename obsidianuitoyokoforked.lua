...
local Library = {
    ...
    IsLightTheme = false,
    Scheme = {
        BackgroundColor = Color3.fromRGB(6, 6, 6),      -- deeper black
        MainColor = Color3.fromRGB(14, 14, 14),          -- darker panels
        AccentColor = Color3.fromRGB(255, 255, 255),     -- pure white (more visible now)
        OutlineColor = Color3.fromRGB(32, 32, 32),       -- darker outlines so white pops
...
function Library:CreateWindow(WindowInfo)
    ...
    do
        ...
        if WindowInfo.Resizable then
            ...
            -- Visual corner grip (looks like the curved handle)
            ...
            -- Inner darker part to give depth
            local GripInner = New("Frame", {
                AnchorPoint = Vector2.new(1, 1),
    ...
    -- Open / Close animations (safe - only animates MainFrame)
    local AnimTweenInfo = TweenInfo.new(0.22, Enum.EasingStyle.Quint, Enum.EasingDirection.Out)
    ...
    function Library:Toggle(Value: boolean?)
        ...
        if Library.Toggled then
            ...
            MainFrame.Size = UDim2.fromOffset(BaseSize.X.Offset * 0.95, BaseSize.Y.Offset * 0.95)
            MainFrame.Position = UDim2.new(
                BasePos.X.Scale,
                BasePos.X.Offset + (BaseSize.X.Offset * 0.025),
                BasePos.Y.Scale,
                BasePos.Y.Offset + (BaseSize.Y.Offset * 0.025)
            )
            MainFrame.Visible = true

            local Tw = TweenService:Create(MainFrame, AnimTweenInfo, {
                Size = BaseSize,
                Position = BasePos,
            })
            Tw:Play()
            ...
            local Tw = TweenService:Create(MainFrame, AnimTweenInfo, {
                Size = UDim2.fromOffset(BaseSize.X.Offset * 0.95, BaseSize.Y.Offset * 0.95),
                Position = UDim2.new(
                    BasePos.X.Scale,
                    BasePos.X.Offset + (BaseSize.X.Offset * 0.025),
                    BasePos.Y.Scale,
...
function Library:SaveConfig(Name)
    ...
    if Success then
        writefile(Library.ConfigFolder .. "/" .. tostring(Name) .. ".json", Encoded)
...
function Library:SaveTheme(Name)
    ...
    if Success then
        writefile(Library.ThemeFolder .. "/" .. tostring(Name) .. ".json", Encoded)
