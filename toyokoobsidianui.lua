...
function Library:CreateWindow(WindowInfo)
    ...
    -- Better Open/Close Animation
    local OpenTweenInfo = TweenInfo.new(0.28, Enum.EasingStyle.Quint, Enum.EasingDirection.Out)
    local CloseTweenInfo = TweenInfo.new(0.22, Enum.EasingStyle.Quint, Enum.EasingDirection.In)
    ...
    function Library:Toggle(Value: boolean?)
        ...
        if not Library.Toggled then
            ...
            for _, Option in pairs(Library.Options) do
                ...
                elseif Option.Type == "Dropdown" or Option.Type == "KeyPicker" then
                    if Option.Menu then Option.Menu:Close() end
                end
            end
        end
        ...
        if Library.Toggled then
            ...
            MainFrame.Size = UDim2.fromOffset(
                TargetSize.X.Offset * 0.92,
                TargetSize.Y.Offset * 0.92
            )
            MainFrame.Position = UDim2.new(
                TargetPos.X.Scale,
                TargetPos.X.Offset + (TargetSize.X.Offset * 0.04),
                TargetPos.Y.Scale,
                TargetPos.Y.Offset + (TargetSize.Y.Offset * 0.04)
            )
            ...
            local CloseTween = TweenService:Create(MainFrame, CloseTweenInfo, {
                ...
                Size = UDim2.fromOffset(
                    CurrentSize.X.Offset * 0.92,
                    CurrentSize.Y.Offset * 0.92
                ),
                Position = UDim2.new(
                    CurrentPos.X.Scale,
                    CurrentPos.X.Offset + (CurrentSize.X.Offset * 0.04),
                    CurrentPos.Y.Scale,
                    CurrentPos.Y.Offset + (CurrentSize.Y.Offset * 0.04)
                )
            })
            CloseTween:Play()
            ...
            for _, Child in pairs(MainFrame:GetDescendants()) do
                if Child:IsA("GuiObject") then
                    TweenService:Create(Child, CloseTweenInfo, {
                        BackgroundTransparency = 1
                    }):Play()
                end
                if Child:IsA("TextLabel") or Child:IsA("TextButton") or Child:IsA("TextBox") then
            ...
            CloseTween.Completed:Connect(function()
                ...
                -- Reset transparencies so next open works cleanly
                ...
                for _, Child in pairs(MainFrame:GetDescendants()) do
                    if Child:IsA("GuiObject") then
                        -- We don't perfectly restore every original value here
                        -- but the next open animation will handle it
                    end
                end
...
function Library:LoadConfig(Name)
    if not readfile or not isfile then
        Library:Notify("readfile not supported on this executor")
        return false
    end

    local Path = Library.ConfigFolder .. "/" .. Name .. ".json"
    if not isfile(Path) then
        Library:Notify({ Title = "Config", Description = "Config not found", Time = 3 })
        return false
    end
    ...
    if not Success or typeof(Data) ~= "table" then
        Library:Notify({ Title = "Config", Description = "Failed to load config", Time = 3 })
        return false
    end
        ...
        for Idx, Value in pairs(Data.Options) do
            if Options[Idx] then
                ...
                if Opt.Type == "ColorPicker" and typeof(Value) == "table" then
                    local Color = Color3.fromHex(Value[1] or "FFFFFF")
                    Opt:SetValueRGB(Color, Value[2] or 0)
                else
                    Opt:SetValue(Value)
                end
            end
...
function Library:SaveTheme(Name)
    ...
    if Success then
        writefile(Library.ThemeFolder .. "/" .. Name .. ".json", Encoded)
        Library:Notify({ Title = "Theme", Description = "Theme \"" .. Name .. "\" saved", Time = 3 })
        return true
    end
    return false
end
...
function Library:LoadTheme(Name)
    ...
    Library:UpdateColorsUsingRegistry()
    Library:Notify({ Title = "Theme", Description = "Theme \"" .. Name .. "\" loaded", Time = 3 })
    return true
end
